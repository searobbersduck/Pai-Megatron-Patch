...
torchrun $DISTRIBUTED_ARGS /workspace/code/llm/pai-megatron-patch-branch/Pai-Megatron-Patch/examples/qwen2_vl/pretrain_qwen.py ${megatron_options} ${dataset_option} ${pr_options} ${load_options} ${activation_checkpoint_options} \
 ${do_options} ${gqa_options} ${sft_option} ${tie_option} ${packing_options} ${uneven_split_option} ${vp_options} ${comm_overlap_option} ${offload_option} \
 --tensorboard-dir ${TENSORBOARD_DIR} \
