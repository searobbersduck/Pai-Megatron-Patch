...
DATASET="llava-datasets"
JOB_ID="0001"
NAME="QWEN2.5-VL-${DATASET}-${JOB_ID}-no-mask"
...
OUT_DIR=./exp/${NAME}
mkdir -p $OUT_DIR
TENSORBOARD_DIR=./exp/${NAME}
mkdir -p $TENSORBOARD_DIR


torchrun $DISTRIBUTED_ARGS /workspace/code/llm/pai-megatron-patch-branch/Pai-Megatron-Patch/examples/qwen2_vl/pretrain_qwen_2_5_vl.py ${megatron_options} ${dataset_option} ${pr_options} ${load_options} ${activation_checkpoint_options} \
 ${do_options} ${gqa_options} ${sft_option} ${tie_option} ${packing_options} ${uneven_split_option} ${vp_options} ${comm_overlap_option} ${offload_option} \
 --tensorboard-dir ${TENSORBOARD_DIR} \
 --wandb-project ${WANDB_PROJECT} \
 --wandb-exp-name ${NAME} \
 --wandb-save-dir $OUT_DIR
#  --wandb-save-dir $OUT_DIR 1>$OUT_DIR/training.log 2>$OUT_DIR/training.err