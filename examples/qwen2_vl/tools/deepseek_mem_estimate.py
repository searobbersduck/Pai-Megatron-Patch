# copy from https://gitlab-master.nvidia.com/arch_moe_exploration/megatron-moe-scripts/-/blob/master/misc/tools/deepseek_mem_estimate.py?ref_type=heads
...
def calc(name, seq_len,
    ...
    if not fp8:
        ...
        q_norm_out = seq_len * 1 * q_lora_rank / tp * bf16_mb_coeff
        ...
        kv_norm_out = seq_len * 1 * kv_lora_rank * bf16_mb_coeff
        ...
        cached_after_up_proj_recompute = cached_after_probs2swiglu - up_proj_save_t
        print(f' --- Cached size after the above recomputations: {cached_after_up_proj_recompute / 1024} GB')
        print(f' --- total usage {rank_dense_mem + rank_moe_mem + cached_after_up_proj_recompute / 1024} GB')
        print()
        ...
        cached_after_recompute = cached_after_up_proj_recompute - act_func_save_t - norm_save_t
        ...
        input_mem = seq_len * 1 * n_embed / tp * bf16_mb_coeff
        input_norm_out = seq_len * 1 * n_embed / tp * bf16_mb_coeff
        q_down_in = seq_len * 1 * n_embed / tp * fp8_mb_coeff
        q_down_out = seq_len * 1 * q_lora_rank / tp * bf16_mb_coeff
        kv_down_in = q_down_in
        kv_down_out = seq_len * 1 * (kv_lora_rank + rope_head_dim) / tp * bf16_mb_coeff
        kv_compressed = seq_len * 1 * kv_lora_rank / tp * bf16_mb_coeff
        kv_pos_embed = seq_len * 1 * rope_head_dim / tp * bf16_mb_coeff
        q_norm_out = seq_len * 1 * q_lora_rank / tp * bf16_mb_coeff
        q_up_in = seq_len * 1 * q_lora_rank / tp * fp8_mb_coeff
        q_up_out = seq_len * 1 * n_head * qk_head_dim / tp * bf16_mb_coeff
        kv_norm_out = seq_len * 1 * kv_lora_rank * bf16_mb_coeff
        kv_up_in = seq_len * 1 * kv_lora_rank * fp8_mb_coeff
        kv_up_out = seq_len * 1 * n_head * (qk_head_dim - rope_head_dim + v_head_dim) / tp * bf16_mb_coeff
        q_apply_rope_out = q_up_out
        k_apply_rope_out = seq_len * 1 * n_head * qk_head_dim / tp * bf16_mb_coeff
        v_apply_rope_out = seq_len * 1 * n_head * v_head_dim / tp * bf16_mb_coeff
        attn_out = seq_len * 1 * n_head * v_head_dim / tp * bf16_mb_coeff
        attn_ctx_tensor = 1 * n_head / tp * seq_len * 1 * fp32_mb_coeff
        proj_in = seq_len * 1 * n_head * v_head_dim / tp * fp8_mb_coeff
        proj_out = seq_len * 1 * n_embed / tp * bf16_mb_coeff
        attn_bda_out = proj_out
        mlp_norm_out = seq_len * 1 * n_embed / tp * bf16_mb_coeff
        ...
        cached = input_mem + q_down_in + q_down_out + kv_down_in + kv_compressed + q_up_in + kv_up_in + q_apply_rope_out + k_apply_rope_out + v_apply_rope_out + attn_out + attn_ctx_tensor + \
        ...
        print(f' -- q_down_in: {q_down_in} MB cached by q_down            {q_down_in / cached * 100:.2f}%')
        print(f' -- q_down_out: {q_down_out} MB cached by q_norm            {q_down_out / cached * 100:.2f}%')
        print(f' -- kv_down_in: {kv_down_in} MB cached by kv_down            {kv_down_in / cached * 100:.2f}%')
        print(f' -- kv_down_out: {kv_down_out} MB not cached')
        print(f' -- kv_compressed (output of split(kv_down_out)): {kv_compressed} MB cached by kv_norm            {kv_compressed / cached * 100:.2f}%')
        print(f' -- kv_pos_embed (output of split(kv_down_out)): {kv_pos_embed} MB not cached')
        print(f' -- q_norm_out: {q_norm_out} MB not cached')
        print(f' -- q_up_in: {q_up_in} MB cached by q_up (can be saved by up_proj recompute)            {q_up_in / cached * 100:.2f}%')
        print(f' -- q_up_out: {q_up_out} MB not cached')
        print(f' -- kv_norm_out: {kv_norm_out} MB not cached')
        print(f' -- kv_up_in: {kv_up_in} MB cached by kv_up (can be saved by up_proj recompute)            {kv_up_in / cached * 100:.2f}%')
        print(f' -- kv_up_out: {kv_up_out} MB not cached')
        print(f' -- q_apply_rope_out: {q_apply_rope_out} MB cached by core_attn (can be saved by up_proj recompute)            {q_apply_rope_out / cached * 100:.2f}%')
        print(f' -- k_apply_rope_out: {k_apply_rope_out} MB cached by core_attn (can be saved by up_proj recompute)            {k_apply_rope_out / cached * 100:.2f}%')
        print(f' -- v_apply_rope_out: {v_apply_rope_out} MB cached by core_attn (can be saved by up_proj recompute)            {v_apply_rope_out / cached * 100:.2f}%')
        ...
        up_proj_save = q_apply_rope_out + k_apply_rope_out + v_apply_rope_out - kv_pos_embed + q_up_in + kv_up_in
        # up_proj_save += q_down_out + kv_compressed + kv_pos_embed + attn_out + attn_ctx_tensor + proj_in
        up_proj_save_t = up_proj_save * cached_layer_num
        print(f' --- By up_proj+rope recompute, can save {up_proj_save} MB')
        print(f' --- By up_proj+rope recompute, can save {up_proj_save_t / 1024} GB for all PP microbatches')
        cached_after_up_proj_recompute = cached_t - up_proj_save_t
        print(f' --- Cached size after the above recomputations: {cached_after_up_proj_recompute / 1024} GB')
        print(f' --- total usage {rank_dense_mem + rank_moe_mem + cached_after_up_proj_recompute / 1024} GB')
        print()

        act_func_save = expert_linear_2_in + share_linear_2_in
        act_func_save_t = act_func_save * cached_layer_num
        print(f' --- By act_func recompute, can save {act_func_save} MB for 1 layer and 1 micobatch')
        print(f' --- By act_func recompute, can save {act_func_save_t / 1024} GB for all PP microbatches')
        norm_save = q_down_in + kv_down_in + proj_in + mlp_norm_out + share_linear_1_in
        norm_save_t = norm_save * cached_layer_num
        print(f' --- By norm recompute, can save {norm_save} MB')
        print(f' --- By norm recompute, can save {norm_save_t / 1024} GB for all PP microbatches')
        cached_after_recompute = cached_after_up_proj_recompute - act_func_save_t - norm_save_t
